...
__TEMPLATE_CLASS__::probe_with_criticality_update(uint64_t address)
{
    ...
    const cset_t& s = cache_->csets_.at(idx);
...
__TEMPLATE_CLASS__::update_criticality(size_t idx)
{
    ...
    critical_counts_[idx] = count_dirty_lru_ways(s);
}
...
__TEMPLATE_CLASS__::schedule_writeback(size_t channel_idx)
{
    ...
    if (it != critical_counts_.end())
    {
        ...
        update_criticality_after_scheduled_writeback(idx, e_it->address, w);
        wb = *e_it;
    }
    return wb;
}
...
__TEMPLATE_CLASS__::harvest_write_row_hits(uint64_t base_address)
{
    ...
    size_t idx = get_set_index<SETS>(base_address);
    ...
    for (size_t iidx : dram_common_map_.at(key))
    {
        ...
        const cset_t& s = cache_->csets_.at(iidx);
        lru_ways_t w = get_lru_ways(s);
        ...
        {
            out.push_back(*it);
            update_criticality_after_scheduled_writeback(iidx, it->address, w);
        }
        ++lookups;
...
__TEMPLATE_HEADER__ inline void
__TEMPLATE_CLASS__::update_criticality_after_scheduled_writeback(size_t idx, uint64_t address, const lru_ways_t& w)
{
...
__TEMPLATE_CLASS__::get_lru_ways(const CACHE_TYPE::cset_t& s)
{
...
__TEMPLATE_CLASS__::count_dirty_lru_ways(const CACHE_TYPE::cset_t& s)
{
    lru_ways_t w = get_lru_ways(s);
...
__TEMPLATE_CLASS__::set_critical_count(size_t idx, size_t to)
{
    ...
    cnt = to;
    dirty_count_[dram_channel(idx)] += diff;
}
