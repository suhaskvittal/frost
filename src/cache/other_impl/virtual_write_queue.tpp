...
#define __TEMPLATE_CLASS__ VirtualWriteQueue<IMPL,NUM_SETS,NUM_WAYS,NEXT_TYPE>
...
__TEMPLATE_CLASS__::tick()
{
    ...
    if (in_write_mode_ && writeback_queue_.size() < IMPL::WB_QUEUE_SIZE)
    {
        ...
        if (dirty_it != s.end())
        {
            ...
            Transaction wb_trans(NUM_THREADS, nullptr, TransactionType::WRITE, dirty_it->address);
